...
export class ProductBundleListComponent extends LickAppPageComponent implements OnInit, OnDestroy, LickAppBehavior {
  ...
  setBreadCrumb(): void {
    this.crumbs = [
      ...
      { name: "new", link: "/stores/" + this.store_id + "/catalogs/" + this.catalog_id + "/product-bundle/new", active: false },
    ]
  }
  ...
  onNewItem() : void {
    this.router.navigate([ 'stores', this.store_id, 'catalogs', this.catalog_id, 'product-bundles', 'new']);
  }
  ...
  onEdit(data): void {
    this.router.navigate([ 'stores', this.store_id, 'catalogs', this.catalog_id, 'product-bundles', data.id, 'edit'])
  }
