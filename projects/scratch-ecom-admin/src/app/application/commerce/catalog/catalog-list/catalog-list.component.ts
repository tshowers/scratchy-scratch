...
export class CatalogListComponent extends LickAppPageComponent implements OnInit, OnDestroy, LickAppBehavior {
  ...
  onNewItem() : void {
    this.router.navigate([ 'stores', this.store_id, 'catalogs', 'new']);
  }
  ...
  onDetail(data): void {
    this.router.navigate([ 'stores', this.store_id, 'catalogs',  data.id])
  }

  onEdit(data): void {
    this.router.navigate([ 'stores', this.store_id, 'catalogs', data.id, 'edit'])
  }
