...
export class OrderListComponent extends LickAppPageComponent implements OnInit, OnDestroy, LickAppBehavior {
  ...
  setBreadCrumb(): void {
    this.crumbs = [
      ...
      { name: "orders", link: "/stores/" + this.store_id + "/orders", active: true },
      { name: "new", link: "/stores/" + this.store_id + "/orders/new", active: false },
    ]
  }
  ...
  onNewItem() : void {
    this.router.navigate([ 'stores', this.store_id, 'orders', 'new']);
  }
  ...
  onEdit(data): void {
    this.router.navigate([ 'stores', this.store_id, 'orders', data.id, 'edit'])
  }
