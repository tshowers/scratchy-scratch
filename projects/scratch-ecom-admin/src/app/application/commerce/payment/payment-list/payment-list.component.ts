...
export class PaymentListComponent extends LickAppPageComponent implements OnInit, OnDestroy, LickAppBehavior {
  ...
  setBreadCrumb(): void {
    this.crumbs = [
      ...
      { name: "payments", link: "/stores/" + this.store_id + "/payments", active: true },
  ...
  onNewItem() : void {
    this.router.navigate([ 'stores', this.store_id, 'payments', 'new']);
  }
  ...
  onDetail(data): void {
    ...
    this.router.navigate([ 'stores', this.store_id, 'payments',  data.id])
  }

  onEdit(data): void {
    this.router.navigate([ 'stores', this.store_id, 'payments', data.id, 'edit'])
  }
